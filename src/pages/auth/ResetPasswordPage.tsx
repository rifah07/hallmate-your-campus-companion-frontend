...
export default function ResetPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await authService.resetPassword({ universityId, otp, newPassword: password });
      toast({ title: 'Success', description: 'Password has been reset. Please login with your new password.' });
      navigate('/login');
    } catch (error: any) {
      ...
      <div className="flex-1 flex items-center justify-center">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
          <div className="text-center mb-8">
            ...
            <p className="text-muted-foreground mt-1">Enter the 6-digit OTP from your email</p>
          </div>
          <Card className="border-border/50 shadow-lg">
            <CardHeader className="text-center pb-4">
              <CardTitle className="text-xl">New Password</CardTitle>
              <CardDescription>Enter your University ID, OTP, and choose a new password</CardDescription>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  />
                  {otp && !/^\d{6}$/.test(otp) && (
                    <p className="text-xs text-destructive">Must be exactly 6 digits</p>
                  )}
                </div>
                <div className="space-y-2">
