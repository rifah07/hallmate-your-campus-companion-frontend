...
export default function VisitorsLogPage() {
  ...
  const filtered = mockVisitors.filter(v => statusFilter === 'all' || v.status === statusFilter);
  const checkedIn = mockVisitors.filter(v => v.status === 'CHECKED_IN').length;

  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div><h1 className="text-2xl font-bold">Visitors Log</h1><p className="text-muted-foreground">Track hall visitors</p></div>
        <Button onClick={() => navigate('/visitors/new')}><UserPlus className="mr-2 h-4 w-4" />Register Visitor</Button>
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[{ l: 'Total Today', v: mockVisitors.length, c: 'text-primary' }, { l: 'Checked In', v: checkedIn, c: 'text-success' }, { l: 'Checked Out', v: mockVisitors.length - checkedIn, c: 'text-muted-foreground' }, { l: 'Avg Duration', v: '2.5h', c: 'text-teal' }].map(s => (
      ...
      <Card><CardContent className="p-4">
        ...
        <div className="rounded-lg border overflow-hidden"><Table><TableHeader><TableRow><TableHead>Visitor</TableHead><TableHead>Visiting</TableHead><TableHead>Purpose</TableHead><TableHead>Entry</TableHead><TableHead>Status</TableHead><TableHead className="w-10"></TableHead></TableRow></TableHeader>
          <TableBody>{filtered.map(v => (
            <TableRow key={v.id}>
              ...
              <TableCell className="text-sm">{v.studentName}</TableCell>
              ...
              <TableCell>{v.status === 'CHECKED_IN' && <Button variant="ghost" size="sm" onClick={() => toast({ title: 'Checked Out', description: `${v.visitorName} checked out.` })}><LogOut className="h-4 w-4" /></Button>}</TableCell>
            </TableRow>
          ))}</TableBody></Table></div>
      </CardContent></Card>
    </div>
